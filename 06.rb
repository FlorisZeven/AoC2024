...
class GuardGallivantSolver < AoCExerciseSolver
  ...
  def initial_guard_position
    row = @lab_grid.index {|row| row.include?('^') }
    col = @lab_grid[row].index {|char| char == '^'}
  ...
  def solve_part_1
    ...
    until out_of_bounds
      ...
      guard_row, guard_col, direction, out_of_bounds = move_guard(guard_row, guard_col, direction)
    end

    count_positions(marked_grid)
  end
